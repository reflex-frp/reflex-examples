...
ulW :: MonadWidget t m => Dynamic t (MM String) -> m (Dynamic t (MM (Event t Int))) 
...
listW :: MonadWidget t m => Event t String -> m ()
listW e = do
    rec xs <- foldDyn ($) M.empty $ mergeWith (.)  -- live state, updated by two signals
        ...
        zs <- mapDyn (mergeWith (.) . map (fmap $ M.delete) . M.elems) bs  -- merge delete events
    return ()
