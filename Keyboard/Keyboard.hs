...
import GHCJS.DOM.HTMLInputElement  (getSelectionStart,setSelectionStart)
...
inputW buttonE = do
    rec let newStringE = attachWith (\v (c,n) -> (n + 1,insertAt n c v)) cur posCharE
        ...
        performArg ( setSelectionStart element) $ fmap fst newStringE
        posCharE <- performArg (\c -> ((,) c) <$> getSelectionStart element) buttonE
