      ...
      el "div" $ do
        rec gs  <- foldDyn move initialState pos
            ...
            elAttr "h3" ("style" =: "text-align: center") $
                ...
                -- dynText =<<
                --     (return . fmap
                --         (\g -> "Score: " <> (T.pack . show . score) g)) gs
        return ()
    footer
...
cell gs p = el "td" $ do
    ...
    where
      square pos bd
        ...
        | bd A.! pos == Idk = idk
        | otherwise = no
      yes = "src"=: "static/images/ball.svg"   <> "style" =: "display: block"
      no  = "src"=: "static/images/square.svg" <> "style" =: "display: block"
      idk = "src"=: "static/images/ball.svg"
